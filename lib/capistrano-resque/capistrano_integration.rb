              ...
              workers.each_pair do |queue, number_of_workers|
                ...
                number_of_workers.times do
                  ...
                  run("cd #{current_path} && RAILS_ENV=#{rails_env} QUEUE=\"#{queue}\" \
                   PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 #{fetch(:bundle_cmd, "bundle")} exec rake environment resque:work > /dev/null 2>&1 &", 
                      :roles => role)
                  worker_id += 1
                end
              end
...
PIDFILE=./tmp/pids/scheduler.pid BACKGROUND=yes bundle exec rake resque:scheduler > /dev/null 2>&1 &"
            end
